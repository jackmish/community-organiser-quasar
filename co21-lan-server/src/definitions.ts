...
export type Co21LanServerPlugin = {
  startServer(options: LanIdentity): Promise<{ ok: boolean; port?: number; addresses?: string[]; error?: string }>;
  stopServer(): Promise<{ ok: boolean }>;
  status(): Promise<{ listening: boolean; port: number; addresses: string[] }>;
  setTrustedContractDevices(options: { peers: LanTrustedPeer[] }): Promise<{ ok: boolean; count: number }>;
