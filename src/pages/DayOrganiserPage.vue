...
function setEventDateToToday() {
  ...
  calendarBaseDate.value = new Date(); // Reset calendar to today
}
...
const nextSixMonths = computed(() => {
  ...
  for (let i = 1; i <= 6; i++) {
    ...
    months.push({
      label: showYear ? format(futureDate, "MMM yyyy") : format(futureDate, "MMM"),
...
function jumpToMonth(dateString: string) {
  // Simply set the calendar to display this month
  const targetDate = new Date(dateString);
  const targetMonth = targetDate.getMonth();
  const targetYear = targetDate.getFullYear();

  // Use day 1 of the month to ensure we show the start of the month
...
const eventTimeHour = computed(() => {
  ...
  return Number(newTask.value.eventTime.split(":")[0]);
});

const eventTimeMinute = computed(() => {
  if (!newTask.value.eventTime) return "";
  return Number(newTask.value.eventTime.split(":")[1]);
});
...
const parentTaskOptions = computed(() => {
  return currentDayData.value.tasks.map((task) => ({
    label: task.name,
    value: task.id,
