...
// e.g. "8.April" (en) or "8.Kwiecień" (pl)
...
const autoGeneratedName = computed(() => {
  ...
  // If description starts with a dash (list), generate a structured title:
  // "{d.Month} {Task Type} {time mode if TimeEvent}"
  if (/^-/.test(desc)) {
    const dateStr = localNewTask.value.eventDate || CC.task.time.currentDate.value || "";
    const dateLabel = dateStr ? formatTitleDate(dateStr) : "";

    const typeId = localNewTask.value.type_id || "TimeEvent";
    let typeLabel: string;
    if (typeId === "Todo") {
      typeLabel = "Todo";
    } else if (typeId === "Replenish") {
      typeLabel = "Replenish";
  ...
  // Strip any leading dash-list prefix that leaked through
