...
import type { TaskAttachment, TaskEmbeddedContact } from "src/modules/task/models/TaskModel";
import TaskContactRelationControls from "./TaskContactRelationControls.vue";
import {
  normalizeEmbeddedContacts,
  normalizeRelatedContactRefs,
...
const isContactNoteMode = computed(
  () => isNoteLater.value && localNewTask.value.noteMode === "contact",
);

const showContactRelationControls = computed(
  () => !props.mediaMode && props.mode !== "preview",
);

const embeddedContactsModel = computed({
  get: () => localNewTask.value.contacts || [],
  set: (val: TaskEmbeddedContact[]) => {
    localNewTask.value.contacts = val;
  },
});

const gatherContactsEnabledModel = computed({
  get: () => !!localNewTask.value.gatherContactsEnabled,
  set: (val: boolean) => {
    localNewTask.value.gatherContactsEnabled = val;
    if (!val) localNewTask.value.contacts = [];
  },
});
...
watch(
  ...
  (val) => {
    ...
    if (!localNewTask.value.contacts) localNewTask.value.contacts = [];
  },
);
  ...
  (val) => {
    if (val) {
      ...
      localNewTask.value = {
        ...
        contacts: normalizeEmbeddedContacts(val.contacts),
        gatherContactsEnabled: !!val.gatherContactsEnabled,
        relatedContactRefs: normalizeRelatedContactRefs(val.relatedContactRefs),
...
watch(autoGeneratedName, (gen) => {
  if (isContactNoteMode.value) return;
...
function onSubmit(event: Event) {
  ...
  if (isContactNoteMode.value) {
    const contactName = String(localNewTask.value.name || "").trim();
    if (!contactName && localNewTask.value.contacts?.length) {
      localNewTask.value.name = String(localNewTask.value.contacts[0]?.name || "").trim();
    } else if (contactName) {
      localNewTask.value.name = contactName;
    }
  } else {
                ...
                <div class="col">
                  ...
                  <div>
                    <div v-if="isNoteLater" class="q-pa-sm col">
                      ...
                      <template v-else-if="localNewTask.noteMode === 'contact'">
                        <q-input
                          ref="descriptionInput"
                          :model-value="localNewTask.description"
                          :label="$text('task.contact.task_notes_label')"
                          outlined
                          type="textarea"
                          rows="2"
                          class="col"
                    ...
                    <div class="col-auto">
                      <div class="row items-center" style="gap: 8px">
                        <TaskContactRelationControls
                          v-if="showContactRelationControls"
                          :is-contact-task="isContactNoteMode"
                          v-model:gather-contacts-enabled="gatherContactsEnabledModel"
                          v-model:embedded-contacts="embeddedContactsModel"
                          :editable="true"
                        />
                        <q-btn
