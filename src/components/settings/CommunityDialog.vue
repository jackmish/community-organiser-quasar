  ...
  <q-dialog v-model="visible" persistent>
    <q-card style="min-width: 520px; max-width: 94vw">
      ...
      <q-card-section>
        ...
        />
        <div class="text-caption text-grey-6 q-mt-xs">ID: {{ ownDeviceId || "—" }}</div>
      </q-card-section>
      ...
      <q-card-section>
        ...
        <q-list bordered separator v-else>
          <q-expansion-item v-for="p in profiles" :key="p.id" expand-separator>
            <!-- ── Device header slot ──────────────────────────────── -->
            <template #header>
              <q-item-section avatar style="width: 36px; min-width: 36px">
                <q-icon
                  :name="transportIcon(p.transport)"
                  :color="trustColor(p.trustStatus)"
                />
              </q-item-section>
              ...
              <q-item-section>
                ...
                <div class="text-caption text-grey-6 row items-center q-gutter-xs">
                  ...
                  <span>·</span>
                  <span
                    >{{ p.groupLinks.length }} group{{
                      p.groupLinks.length === 1 ? "" : "s"
                    }}</span
                  >
                </div>
              </q-item-section>
              ...
              <q-list dense v-if="p.groupLinks.length">
                ...
                >
                  ...
                  <q-item-section side>
                    <div class="row items-center q-gutter-xs">
                      <q-badge
                        :color="privilegeColor(link.privilege)"
                        :label="link.privilege"
                      />
                      <q-btn
                        flat
                        dense
