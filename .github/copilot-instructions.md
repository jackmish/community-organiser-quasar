...
- **Quasar theme opt-out (`use-default`):** … Use **`getCo21FieldCssVariables` / `getCo21MenuCssVariables`** from `src/components/theme.ts`. CSS selectors must use `.q-field.q-select.use-default.q-field--outlined` (same root element) — not `.q-select.use-default .q-field--outlined` (descendant, never matches).
