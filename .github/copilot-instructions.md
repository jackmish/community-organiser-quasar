...
- at this moment project structure is going to be like:
  ...
  **CC Registry pattern (April 2026):** CC now uses a typed `_lazy()` registry. To add a new domain controller: (1) implement `Controllable` (`src/types/Controllable.ts`), (2) add one `_lazy()` backing field and a typed getter in `CentralController.ts`. `CC.boot()` (called from `boot/pinia.ts` after Pinia is active) automatically wires all storage ports and lifecycle hooks — no other bootstrap changes needed. Storage now receives data via `StorageController.connect(port)` instead of constructor injection.
  **CC split into CC + CCRegistry (April 2026):** `CentralController.ts` now exports two things: `CC` (typed access proxy — default export used by components) and `CCReg` (the `CCRegistry` instance — named export used for boot and optional external registration). Each domain is registered in a single fluent line (e.g. `.register('group', GroupStoreController)`). Adding a new domain = one `.register()` line and one getter; everything else is automatic. The registry engine lives in `src/CCRegistry.ts`. `CCReg.boot()` is called from `boot/pinia.ts` after Pinia is active.
