...
5. As in previous point there are lot of things user is changing manually and AI is treating user changes as mistakes. This time AI written automated tests forcing setting values from the tests. There were so many times when AI was changing color/transparency, reverting user values/code to AI generated code, but this time it even written automated test to not allow user to change any configurations values.

---

##### Back to point 1, its really hard to find good core naming/pattern structure for application

... a specially when ChatGPT5.1 was designing monolith system. Refactor also is not going well, splitting code to smaller parts. Claude is much more capable but also feels a bit same, a specially when code was vibe coded at beginning. Monolith structure and any spaghetti code isn't a problem if app is small, maybe medium, but its a problem also for AI to know what is happening. Processing very long files takes it too much time. In later stage of refactor its much easier to recommend AI something really good or maybe just there is a hope.

Next thing: file/class names/patterns I'm not sure it's going in the good direction, but from time to time i can get at least some tip about patterns. Many things looks like correct - at least at first look, but GPT5.1 is rather not encourages to any refactor attempts, and code structure could look bad for a long time.
